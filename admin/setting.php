    ...
    <div class="row"style="margin-top: 25px;">
        <div style="margin-left: 20px;width: 100%;margin-right: 20px;">
            ...
            <form action="setting.php" method="post">
                ...
                <label>ایمیل: </label>
                <input type="email"name="email"class="form-control col-8"value="<?php echo $result1[0]["email"];?>">
            ...
            if (isset($_POST["submit"])){
                ...
                $data2=$connection->prepare("UPDATE `admin` SET `name`='$name',`lname`='$lname',`username`='$userna',`phone`='$phone',`email`='$email' WHERE username='$usern'");
                $data2->execute();
                $_SESSION["admin"]=$userna;
                echo "<div class='alert alert-success'>تغییرات ذخیره شد. </div>";
            }
            if (isset($_POST["pass_submit"])){
                $pass=$_POST["pass"];
                $data3=$connection->prepare("UPDATE `admin` SET password='$pass' WHERE username='$usern'");
                $data3->execute();
                echo "<div class='alert alert-success'>تغییرات ذخیره شد. </div>";
            }
            ?>
